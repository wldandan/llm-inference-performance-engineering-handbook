...
![Gateway 请求入口](figures/fig01-02_gateway_request_entry.svg)
...
图1-4：Prefill 在生命周期中的位置。

## 1.5 First Token 与 Streaming Response
...
![首个 token 与流式返回](figures/fig01-05_first_token_streaming.svg)

图1-5：首个 token 与流式返回。

## 1.6 Decode Loop：逐 token 生成
...
![KV Cache 生命周期](figures/fig01-07_kv_cache_lifecycle.svg)

图1-7：KV Cache 生命周期。

## 1.8 Response 结束与资源回收
...
![请求结束与资源回收](figures/fig01-08_finish_cleanup.svg)
...
![Demo 字段映射到生命周期](figures/fig01-09_demo_field_mapping.svg)

图1-9：Demo 字段映射到生命周期。

## 1.10 课堂案例：为什么“首包慢”和“输出慢”要分开问
...
![生命周期与后续章节边界](figures/fig01-10_lifecycle_boundaries.svg)
