...
图 1-1 到图 1-10 已按 v2 视觉系统完成，覆盖请求总览、入口、排队、Prefill、首 token、Decode、KV Cache、资源回收、Demo 字段映射和章节边界。图 1-1 突出“客户端发起请求—服务流式返回”，没有扩张成完整 Serving 架构图。

## 修改优先级

1. P0：在目标 GPU 环境执行一次端到端验证并保存环境版本。
2. 已完成：第 2 章提供更正式的生命周期模型，并与本章保持“现象—模型”的分工。
3. 已完成：图 1-1 和图 1-9 可直接承接运行流程与 Demo 输出字段。
...
- 10 张 v2 SVG、配套制作说明与插图视觉契约测试全部通过。
