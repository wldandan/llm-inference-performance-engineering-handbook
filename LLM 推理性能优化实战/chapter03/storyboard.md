...
| 图号 | 标题 | 核心问题 | 状态 |
|---|---|---|---|
| 图3-1 | LLM 推理系统全局架构 | 请求与响应分别经过哪些逻辑组件？ | final v2 |
| 图3-2 | Client 与 Gateway 边界 | 业务调用和服务入口分别负责什么？ | final v2 |
| 图3-3 | 三层流量决策 | Router、Admission、Engine Scheduler 如何分工？ | final v2 |
| 图3-4 | Worker、Runtime 与 Accelerator | 执行计划如何变成硬件计算？ | final v2 |
| 图3-5 | 推理服务的三类状态 | 请求、执行和资源状态由谁拥有？ | final v2 |
| 图3-6 | LLM Serving 部署形态 | 逻辑角色如何映射到不同部署单元？ | final v2 |
| 图3-7 | 成熟系统的架构层级映射 | 平台、推理服务器和 LLM Engine 有何区别？ | final v2 |
| 图3-8 | 从现象到组件和观测点 | 架构图如何指导证据采集？ | final v2 |
| 图3-9 | 架构契约 Demo | JSON 如何变成可验证的路径与部署报告？ | final v2 |
| 图3-10 | Chapter 3 的章节边界 | 架构与生命周期、模型机制、硬件和指标如何分工？ | final v2 |
...
- 每张 SVG 都包含可访问标题、说明和 `chapter03-v2` 视觉契约标识，并配套 `figure-note.md`。
