# 第 4 章 Storyboard：Transformer 推理机制

本章插图只解释模型机制，不引入 GPU 性能结论、指标阈值或优化技术。

| 图号 | 标题 | 核心问题 | 状态 |
|---|---|---|---|
| 图4-1 | 从文本到下一个 token | 一次 token 生成经过哪些模型阶段？ | final v2 |
| 图4-2 | Decoder-only Transformer Block | Attention、MLP、Norm 与 Residual 怎样连接？ | final v2 |
| 图4-3 | Causal Self-Attention | 一个位置怎样读取允许访问的历史？ | final v2 |
| 图4-4 | MHA 与 GQA | Query Heads 与 KV Heads 的数量为何不同？ | final v2 |
| 图4-5 | Prefill | 完整 Prompt 怎样产生首个输出 token？ | final v2 |
| 图4-6 | Sampling | Logits 怎样变成具体 token？ | final v2 |
| 图4-7 | Decode 与 KV Cache | 每个生成步骤怎样复用并追加历史状态？ | final v2 |
| 图4-8 | Demo 输出模型机制报告 | 合成报告怎样串起形状、采样与执行事件？ | final v2 |
