...
EXPECTED_FIGURES = {
    "fig04-01_token_generation_pipeline.svg": "从文本到下一个 token",
    "fig04-02_decoder_block.svg": "Decoder-only Transformer Block",
    "fig04-03_causal_self_attention.svg": "Causal Self-Attention",
    "fig04-04_gqa_heads.svg": "MHA 与 GQA",
    "fig04-05_prefill_execution.svg": "Prefill",
    "fig04-06_sampling_pipeline.svg": "Sampling",
    "fig04-07_decode_kv_loop.svg": "Decode 与 KV Cache",
    "fig04-08_demo_mechanics_report.svg": "Demo 输出模型机制报告",
}
...
class ChapterFourFigureTests(unittest.TestCase):
    ...
    def test_every_figure_uses_visual_contract(self):
        for filename, expected_title in EXPECTED_FIGURES.items():
            with self.subTest(filename=filename):
                ...
                self.assertEqual(root.attrib.get("data-figure-system"), "chapter04-v2")
    ...
    def test_chapter_links_eight_continuous_figures(self):
        source = (CHAPTER_DIR / "chapter04.md").read_text(encoding="utf-8")
        links = re.findall(r"!\[[^\]]*\]\((figures/[^)]+\.svg)\)", source)
        self.assertEqual(links, [f"figures/{name}" for name in EXPECTED_FIGURES])
        self.assertEqual(re.findall(r"图4-(\d+)[：:]", source), [str(i) for i in range(1, 9)])
