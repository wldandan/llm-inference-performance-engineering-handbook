...
完成本章后，学员应能够把「现象、指标、瓶颈、优化动作」连接起来，而不是只记住单个工具或参数。本章重点围绕 `Prefill Profiling` 建立可操作的分析框架。
...
2. 运行本章 `./ch04/demo.py` 生成一组可复现的模拟指标。
...
- [ ] 能解释：为什么 Prefill 是 Compute Bound
- [ ] 能解释：Prompt 长度对 TTFT 的影响
- [ ] 能解释：Attention 与 GEMM 的计算特点
