...
完成本章后，学员应能够把「现象、指标、瓶颈、优化动作」连接起来，而不是只记住单个工具或参数。本章重点围绕 `Continuous Batching` 建立可操作的分析框架。

## 核心内容

- 请求级动态插入
- Decode 阶段调度
- 普通 Batch 与 Continuous Batch
...
2. 运行本章 `./ch13/demo.py` 生成一组可复现的模拟指标。
...
- [ ] 能解释：请求级动态插入
- [ ] 能解释：Decode 阶段调度
- [ ] 能解释：普通 Batch 与 Continuous Batch
