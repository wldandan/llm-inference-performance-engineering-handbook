...
第 2 章讲完请求生命周期后，本章把生命周期转成指标语言。指标不是为了做漂亮报表，而是为了把“慢”“卡”“贵”“不稳定”这些模糊反馈变成可比较、可复现的问题。
...
![推理性能指标全景](figures/fig03-01_metrics_overview.svg)
...
![TTFT 时间范围](figures/fig03-02_ttft_scope.svg)
...
![TPOT 与 ITL](figures/fig03-03_tpot_itl.svg)

图3-3：TPOT 与 ITL。

## 3.4 TPS 与 RPS：吞吐不是一种指标
...
![TPS 与 RPS 的区别](figures/fig03-04_tps_rps.svg)
...
![分位数与尾延迟](figures/fig03-05_percentile_latency.svg)

图3-5：分位数与尾延迟。

## 3.6 GPU Utilization 与 GPU Memory
...
本章只讲指标意义。第 6 章会讲如何用工具采集，第 21 到第 24 章会讨论容量和扩展。

![GPU 利用率与显存](figures/fig03-06_gpu_metrics.svg)

图3-6：GPU 利用率与显存。

## 3.7 Cost per Token：性能最终要回到成本
...
![Cost per Token](figures/fig03-07_cost_per_token.svg)
...
![指标 Trade-off](figures/fig03-08_metrics_tradeoff.svg)

图3-8：指标 Trade-off。

## 3.9 Demo：采集一次客户端指标

本章可以继续使用 `chapter01/demo/demo.py` 做一个小实验。它不是完整 benchmark，只是帮你理解指标字段。
...
这里的请求数很少，不足以得出生产结论。它只用于理解字段。可信 benchmark 要在第 5 章建立。

![Demo 指标输出](figures/fig03-09_demo_metrics_output.svg)

图3-9：Demo 指标输出。

## 3.10 课堂案例：同一个系统，为什么两份报告结论相反
...
![指标口径检查](figures/fig03-10_metric_checklist.svg)
