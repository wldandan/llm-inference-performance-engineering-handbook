# 第 4 章 Global Performance Model
...
![全局性能模型](figures/fig04-01_global_performance_model.svg)
...
![Queue 对延迟的影响](figures/fig04-02_queue_latency.svg)

图4-2：Queue 对延迟的影响。

## 4.3 Prefill：首包前的大块计算
...
Prefill 常与 Compute Bottleneck 相关，但不能机械判断。是否 compute bound，需要看 GPU timeline、SM / Tensor Core 利用率、HBM 带宽和 kernel 结构。第 10 章会专门分析 Prefill 性能。
...
![Prefill 对首包的影响](figures/fig04-03_prefill_latency.svg)

图4-3：Prefill 对首包的影响。

## 4.4 Decode：输出长度驱动的循环
...
Decode 常与 Memory Bottleneck 相关，因为每一步都要读取历史 KV Cache。但真实系统还可能受调度、kernel launch、采样和网络发送影响。第 14 到第 16 章会深入 Decode 分析和验证。
...
![Decode 对总延迟的影响](figures/fig04-04_decode_latency.svg)

图4-4：Decode 对总延迟的影响。

## 4.5 Compute Bottleneck
...
![Compute Bottleneck](figures/fig04-05_compute_bottleneck.svg)

图4-5：Compute Bottleneck。
...
![Memory Bottleneck](figures/fig04-06_memory_bottleneck.svg)

图4-6：Memory Bottleneck。

## 4.7 Scheduling Bottleneck
...
![Scheduling Bottleneck](figures/fig04-07_scheduling_bottleneck.svg)

图4-7：Scheduling Bottleneck。
...
![指标因果关系](figures/fig04-08_metric_causality.svg)

图4-8：指标因果关系。

## 4.9 Demo：把一次结果放入全局模型
...
![Demo 结果进入全局模型](figures/fig04-09_demo_to_model.svg)

图4-9：Demo 结果进入全局模型。

## 4.10 课堂案例：首包慢时，先提出三个假设
...
![从模型到验证](figures/fig04-10_model_to_validation.svg)
