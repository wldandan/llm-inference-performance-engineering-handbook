...
![benchmark_overview](figures/fig05-01_benchmark_overview.svg)

图5-1：Benchmark 不是跑分，而是实验设计。

## 5.1 Benchmark 不是跑分，而是实验设计

Benchmark 不是跑分，而是实验设计 是本章的起点。性能分析不是为了得到一个漂亮数字，而是为了让团队知道当前系统在什么工作负载下、用什么指标、以什么重复方式表现如何。没有这个前提，后续任何优化收益都可能只是偶然波动。

![baseline_contract](figures/fig05-02_baseline_contract.svg)

图5-2：Benchmark 不是跑分，而是实验设计。

## 5.2 Baseline：所有优化的参照物

在本书语境里，Baseline：所有优化的参照物 要写清楚输入条件、版本、模型、硬件、并发、请求分布和观测指标。它不是一次命令输出，而是一份可以被别人复现的实验约定。

![warmup_curve](figures/fig05-03_warmup_curve.svg)

图5-3：Baseline：所有优化的参照物。

## 5.3 Warmup：让系统进入稳定状态

Warmup：让系统进入稳定状态 的价值在于排除冷启动、缓存填充、编译、连接建立和一次性初始化带来的干扰。LLM 服务里，第一次请求往往不能代表稳定运行状态。

![repeat_distribution](figures/fig05-04_repeat_distribution.svg)

图5-4：Warmup：让系统进入稳定状态。

## 5.4 Repeat：把偶然波动变成可信区间

Repeat：把偶然波动变成可信区间 让我们知道结论是否稳定。只报告一次运行结果，会把系统抖动、邻居负载、网络波动和调度偶然性混进分析结论。

![prompt_output_matrix](figures/fig05-05_prompt_output_matrix.svg)

图5-5：Repeat：把偶然波动变成可信区间。

## 5.5 Prompt / Output 设计

Prompt / Output 设计 决定请求更像 Prefill 压力还是 Decode 压力。长 prompt 会放大首包前计算，长 output 会放大逐 token 生成，二者不能用同一组结论互相替代。

![concurrency_ladder](figures/fig05-06_concurrency_ladder.svg)

图5-6：Prompt / Output 设计。

## 5.6 Concurrency 设计

Concurrency 设计 影响队列、batch 形成、GPU 利用率和尾延迟。并发不是越高越好，可信实验要说明并发梯度如何设置，以及每个梯度回答什么问题。

![metric_collection](figures/fig05-07_metric_collection.svg)
...
![result_record](figures/fig05-08_result_record.svg)

图5-8：指标采集与结果记录。

## 5.8 Demo：设计一次最小 Benchmark
...
![benchmark_boundary](figures/fig05-10_benchmark_boundary.svg)

图5-10：Benchmark Design 的章节边界。
