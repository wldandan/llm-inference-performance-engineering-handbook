...
![diagnosis_overview](figures/fig08-01_diagnosis_overview.svg)
...
![ttft_checklist](figures/fig08-02_ttft_checklist.svg)
...
![tpot_checklist](figures/fig08-03_tpot_checklist.svg)

图8-3：TTFT Checklist。

## 8.3 TPOT / ITL Checklist

TPOT / ITL Checklist 关注逐 token 生成节奏。它要看 Decode kernel、KV Cache 读取、sampling、streaming 返回、batch 中其他请求的影响，以及长输出下的稳定性。

![tps_checklist](figures/fig08-04_tps_checklist.svg)

图8-4：TPOT / ITL Checklist。

## 8.4 TPS Checklist

TPS Checklist 关注系统总体产出能力。要同时看请求分布、并发梯度、batch 形成、GPU 利用率、平均输出长度和尾延迟，避免为了吞吐牺牲交互体验。

![gpu_util_checklist](figures/fig08-05_gpu_util_checklist.svg)

图8-5：TPS Checklist。

## 8.5 GPU Utilization Checklist

GPU Utilization Checklist 不能只问高低，还要问是否有效忙。高利用率要结合 TPS 和 TPOT，低利用率要结合队列、CPU、Scheduler 和网络。

![memory_checklist](figures/fig08-06_memory_checklist.svg)

图8-6：GPU Utilization Checklist。
...
![priority_matrix](figures/fig08-07_priority_matrix.svg)

图8-7：Memory Checklist。

## 8.7 从 Checklist 到行动优先级

Checklist 的结果要转成行动优先级。能快速排除的先排除，影响面最大的先确认，采集成本低的先采集。不要同时展开十个方向，否则诊断会变成噪声。

![diagnosis_report](figures/fig08-08_diagnosis_report.svg)

图8-8：从 Checklist 到行动优先级。

## 8.8 Demo：填写一次 Performance Diagnosis 表
...
![diagnosis_boundary](figures/fig08-10_diagnosis_boundary.svg)

图8-10：Performance Diagnosis 的章节边界。
...
下一步：按 Chapter 5 重新构造长短 prompt workload，按 Chapter 6 采集 Timeline。
```
