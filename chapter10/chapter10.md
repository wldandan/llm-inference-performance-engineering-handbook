...
![从 TTFT 回到 Prefill](figures/fig10-01_ttft_to_prefill.svg)

图10-1：从 TTFT 回到 Prefill。

## 10.1 从 TTFT 回到 Prefill

TTFT 高不等于 Prefill 一定慢，但 Prefill 是最常见方向之一。要先从第 8 章的诊断结果确认 queue、Gateway 和 Decode 不是主要问题，再进入 Prefill 分析。

从前两篇的角度看，本章不是孤立技术点。Part 1 给出系统位置，Part 2 给出证据方法，本篇开始把二者落到 Prefill 这个具体阶段。

![从 TTFT 回到 Prefill](figures/fig10-02_compute_bound_intuition.svg)

图10-2：从 TTFT 回到 Prefill。

## 10.2 Compute Bound 的直觉
...
![Compute Bound 的直觉](figures/fig10-03_roofline_model.svg)

图10-3：Compute Bound 的直觉。

## 10.3 Roofline：算力与带宽的边界

Roofline 用 operational intensity 把计算量和内存访问放在同一张图里。Prefill 分析中，它帮助判断优化应该朝提高算力利用，还是减少访存和数据搬运。

分析时要同时问三件事：输入是什么，输出是什么，证据在哪里。只讲原理而没有输入输出边界，后续就无法做 Benchmark 和 Profiling。

![Roofline：算力与带宽的边界](figures/fig10-04_tensor_core_utilization.svg)

图10-4：Roofline：算力与带宽的边界。

## 10.4 Tensor Core 利用率

如果矩阵形状、精度和 kernel 实现没有很好匹配 Tensor Core，Prefill 可能表现为算力没有打满。此时单看 GPU Utilization 不够，要看 kernel 级证据。

分析时要同时问三件事：输入是什么，输出是什么，证据在哪里。只讲原理而没有输入输出边界，后续就无法做 Benchmark 和 Profiling。

![Tensor Core 利用率](figures/fig10-05_kernel_timeline.svg)

图10-5：Tensor Core 利用率。

## 10.5 Kernel Timeline

Timeline 能显示 Prefill 的 kernel 顺序、空洞、launch 间隔和 CPU/GPU 协同。它可以证明问题是在大 kernel 内部，还是 kernel 之间有调度空洞。

分析时要同时问三件事：输入是什么，输出是什么，证据在哪里。只讲原理而没有输入输出边界，后续就无法做 Benchmark 和 Profiling。

![Kernel Timeline](figures/fig10-06_prompt_length_distribution.svg)

图10-6：Kernel Timeline。
...
![Prompt 长度分布](figures/fig10-07_profiling_evidence_stack.svg)
...
![Profiling 证据组合](figures/fig10-08_demo_prompt_ladder.svg)

图10-8：Profiling 证据组合。

## 10.8 Demo：构造 Prompt 长度阶梯
...
这段报告仍然不是优化结论。它只是把第 5 章 Benchmark 和第 6 章 Profiling 的方法带入 Prefill，让第 11 章选择优化技术时有证据基础。

## 10.9 课堂案例：长 prompt 是否一定需要优化
...
![课堂案例：长 prompt 是否一定需要优化](figures/fig10-09_long_prompt_case.svg)

图10-9：课堂案例：长 prompt 是否一定需要优化。
...
![Prefill 性能分析 的章节边界](figures/fig10-10_prefill_analysis_boundary.svg)

图10-10：Prefill 性能分析 的章节边界。
