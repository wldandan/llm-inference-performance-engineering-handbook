...
![从 TPOT 回到 Decode](figures/fig14-01__tpot__decode.svg)

图14-1：从 TPOT 回到 Decode。

## 14.1 从 TPOT 回到 Decode

从 TPOT 回到 Decode 是本章的核心内容之一。写作时先说明它在 Decode 流程中的位置，再说明输入、输出、关键状态和可观测证据。这样后续才能把指标变化和具体机制连起来。 这一节负责建立边界，不急着下优化结论。

![Memory Bound 的直觉](figures/fig14-02_memory_bound_.svg)

图14-2：Memory Bound 的直觉。

## 14.2 Memory Bound 的直觉
...
![HBM Bandwidth](figures/fig14-03_hbm_bandwidth.svg)
...
![Decode Timeline](figures/fig14-04_decode_timeline.svg)

图14-4：Decode Timeline。

## 14.4 Decode Timeline
...
![KV Cache Growth](figures/fig14-05_kv_cache_growth.svg)

图14-5：KV Cache Growth。

## 14.5 KV Cache Growth
...
![Batch 中长短输出混合](figures/fig14-06_batch_.svg)
...
![Profiling 证据组合](figures/fig14-07_profiling_.svg)

图14-7：Profiling 证据组合。

## 14.7 Profiling 证据组合

Profiling 证据组合 是本章的核心内容之一。写作时先说明它在 Decode 流程中的位置，再说明输入、输出、关键状态和可观测证据。这样后续才能把指标变化和具体机制连起来。 这一节要讨论指标之间的耦合关系，尤其是 TTFT、TPOT、TPS、GPU Utilization、Memory、Cost 和尾延迟。

![Demo：构造长输出实验](figures/fig14-08_demo.svg)
...
![课堂案例：TPOT 变差但 TTFT 正常](figures/fig14-09_tpot__ttft_.svg)

图14-9：课堂案例：TPOT 变差但 TTFT 正常。
...
![Decode 性能分析 的章节边界](figures/fig14-10_figure.svg)

图14-10：Decode 性能分析 的章节边界。
