...
![优化前先绑定 Decode Root Cause](figures/fig15-01__decode_root_cause.svg)

图15-1：优化前先绑定 Decode Root Cause。

## 15.1 优化前先绑定 Decode Root Cause

优化前先绑定 Decode Root Cause 是本章的核心内容之一。写作时先说明它在 Decode 流程中的位置，再说明输入、输出、关键状态和可观测证据。这样后续才能把指标变化和具体机制连起来。 这一节负责建立边界，不急着下优化结论。

![KV Cache](figures/fig15-02_kv_cache.svg)

图15-2：KV Cache。

## 15.2 KV Cache

KV Cache 是本章的核心内容之一。写作时先说明它在 Decode 流程中的位置，再说明输入、输出、关键状态和可观测证据。这样后续才能把指标变化和具体机制连起来。 这一节要和 Part 2 的 Profiling / Root Cause 方法连接，说明哪些证据支持当前判断。

![PagedAttention](figures/fig15-03_pagedattention.svg)

图15-3：PagedAttention。

## 15.3 PagedAttention

PagedAttention 是本章的核心内容之一。写作时先说明它在 Decode 流程中的位置，再说明输入、输出、关键状态和可观测证据。这样后续才能把指标变化和具体机制连起来。 这一节要和 Part 2 的 Profiling / Root Cause 方法连接，说明哪些证据支持当前判断。

![KV Quantization](figures/fig15-04_kv_quantization.svg)

图15-4：KV Quantization。

## 15.4 KV Quantization
...
![Prefix Cache](figures/fig15-05_prefix_cache.svg)

图15-5：Prefix Cache。

## 15.5 Prefix Cache

Prefix Cache 是本章的核心内容之一。写作时先说明它在 Decode 流程中的位置，再说明输入、输出、关键状态和可观测证据。这样后续才能把指标变化和具体机制连起来。 这一节要讨论指标之间的耦合关系，尤其是 TTFT、TPOT、TPS、GPU Utilization、Memory、Cost 和尾延迟。

![Speculative Decoding](figures/fig15-06_speculative_decoding.svg)

图15-6：Speculative Decoding。

## 15.6 Speculative Decoding

Speculative Decoding 是本章的核心内容之一。写作时先说明它在 Decode 流程中的位置，再说明输入、输出、关键状态和可观测证据。这样后续才能把指标变化和具体机制连起来。 这一节要讨论指标之间的耦合关系，尤其是 TTFT、TPOT、TPS、GPU Utilization、Memory、Cost 和尾延迟。

![MTP / Medusa / EAGLE](figures/fig15-07_mtp___medusa___eagle.svg)

图15-7：MTP / Medusa / EAGLE。

## 15.7 MTP / Medusa / EAGLE

MTP / Medusa / EAGLE 是本章的核心内容之一。写作时先说明它在 Decode 流程中的位置，再说明输入、输出、关键状态和可观测证据。这样后续才能把指标变化和具体机制连起来。 这一节要讨论指标之间的耦合关系，尤其是 TTFT、TPOT、TPS、GPU Utilization、Memory、Cost 和尾延迟。

![Demo：从 TPOT profile 选择候选优化](figures/fig15-08_demo_tpot_profile_.svg)

图15-8：Demo：从 TPOT profile 选择候选优化。

## 15.8 Demo：从 TPOT profile 选择候选优化

本章 Demo 使用固定 workload 或模拟数据展示方法路径。它的作用是训练分析流程，不替代真实生产环境下的 Benchmark 结论。


## 15.9 课堂案例：Speculative Decoding 为什么不总是有效
...
![课堂案例：Speculative Decoding 为什么不总是有效](figures/fig15-09_speculative_decoding_.svg)

图15-9：课堂案例：Speculative Decoding 为什么不总是有效。
...
![Decode 优化方法 的章节边界](figures/fig15-10_figure.svg)

图15-10：Decode 优化方法 的章节边界。
...
本章回答了“面对 Decode 瓶颈应该选择哪些优化方法？”这个问题。它延续前 3 篇的经验，把系统位置、分析证据和优化/验证闭环放到 Decode 场景中。
