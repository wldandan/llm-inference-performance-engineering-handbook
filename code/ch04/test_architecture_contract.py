...
class ArchitectureContractTest(unittest.TestCase):
    ...
    def test_single_instance_profile_does_not_require_router_or_admission(self):
        architecture_contract = self.load_module()
        spec = {
            "name": "minimal-single-instance",
            "profile": "single_instance",
            "components": [
                {"id": "client", "role": "client", "deployment_unit": "caller"},
                {"id": "gateway", "role": "gateway", "deployment_unit": "api-process"},
                {"id": "scheduler", "role": "scheduler", "deployment_unit": "engine-core"},
            ...
            "flows": [
                {"from": "client", "to": "gateway", "kind": "request"},
                {"from": "gateway", "to": "scheduler", "kind": "request"},
                {"from": "scheduler", "to": "worker", "kind": "request"},
                {"from": "worker", "to": "runtime", "kind": "request"},
                {"from": "runtime", "to": "accelerator", "kind": "request"},
                {"from": "accelerator", "to": "runtime", "kind": "response"},
        ...
        report = architecture_contract.validate_architecture(spec)

        self.assertEqual(report["profile"], "single_instance")
        self.assertEqual(
            report["request_path"],
            ["client", "gateway", "scheduler", "worker", "runtime", "accelerator"],
        )
        self.assertEqual(report["optional_roles_absent"], ["router", "admission"])
    ...
    def test_bundled_scale_out_example_is_valid(self):
        architecture_contract = self.load_module()
        input_path = Path(__file__).with_name("scale-out-architecture.json")
        self.assertTrue(input_path.exists(), "scale-out example has not been created")

        report = architecture_contract.validate_architecture(
            architecture_contract.load_spec(input_path)
        )

        self.assertEqual(report["profile"], "scale_out")
        self.assertIn("router", report["request_path"])
        self.assertIn("admission", report["request_path"])
