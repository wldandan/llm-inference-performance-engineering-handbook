"""Chapter 01 demo: send real requests to a vLLM OpenAI-compatible server.

This script measures client-observed TTFT, ITL, total latency, throughput,
and optional GPU snapshots. It does not mock model metrics.
"""
...
class MetricsCollector:
    ...
    def finish(self, end_time: float) -> dict[str, Any]:
        ...
        return {
            ...
            "itl_p50_ms": round(percentile(intervals, 50), 2) if intervals else None,
...
def summarize(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "ttft_avg_ms": round(sum(values("ttft_ms")) / len(values("ttft_ms")), 2)
        if values("ttft_ms")
        else None,
        "ttft_p95_ms": round(percentile(values("ttft_ms"), 95), 2) if values("ttft_ms") else None,
        "itl_avg_ms": round(sum(values("itl_avg_ms")) / len(values("itl_avg_ms")), 2)
        if values("itl_avg_ms")
        else None,
        "total_latency_avg_ms": round(sum(values("total_latency_ms")) / len(values("total_latency_ms")), 2)
        if values("total_latency_ms")
        else None,
        "tokens_per_second_sum": round(sum(values("tokens_per_second")), 2)
        if values("tokens_per_second")
        else None,
