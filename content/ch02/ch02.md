...
第 1 章的流式调用让“多久看到第一个字”成了一个能测的量。现在假设这个量偏大——用户抱怨要等很久才看到回应。你该去改什么？

请求可能在队列里等了太久，可能模型处理这段 prompt 本身就慢，也可能首 token 早已生成、时间全耗在了返回路上。三种原因对应三种完全不同的修法，而客户端记录的那几个事件无法区分它们：它们都发生在请求发出与首个分片到达之间，被压缩成了同一段时间。

要拆开这段时间，得知道请求在服务端内部经历了什么。

本章补上服务端这一侧：先用一套与框架无关的事件和时间点描述请求在系统内部经历了什么，再从同一次真实 vLLM 请求上采集请求级指标和 Prometheus 增量，并为每个结论标明证据来自哪里。Transformer 为什么会产生 Prefill 和 Decode 留到第 3 章，组件由谁实现留到第 4 章，TTFT、TPOT 和分位数的正式定义留到第 5 章。

框架内部还有更细的事件——抢占、Chunked Prefill、远程 KV 传输、Prefill/Decode 分离——它们会让请求路径变得不连续，本章不展开。本章 Demo 只覆盖无抢占的主路径，不能用它计算多次调度和恢复之后的内部区间。
...
vLLM 暴露 Queue、Prefill、Decode、TTFT 和 E2E 等服务端指标，但其边界可能随引擎版本以及抢占行为变化。本章固定无抢占主路径，并记录实际 vLLM 版本；遇到抢占时只报告“当前 Demo 不支持归因”，不强行套用一次 `scheduled` 的公式。[vLLM Production Metrics](https://docs.vllm.ai/en/latest/usage/metrics/)
...
第 1 章数过客户端收到多少个内容分片。现在换个问题：模型生成了多少个 token？

两者不能互相换算。模型产出 token 后，服务端还要解码文本、组装协议对象、写入网络，这几步都可能改变分片与 token 的对应关系——服务端可能攒够几个 token 才发一个分片，网络栈可能把多次写入合并，投机解码还可能一次接受多个 token。分片数是网络行为的结果，token 数是模型行为的结果。

所以同一条时间线上有四个不同的事件，必须分开记录：
...
第 1 章能观测到的只有后两个，前两个发生在服务端。这就是为什么客户端首包等待不能改名叫 TTFT——它包含了从首 token 生成到分片到达客户端的全部路程，而这段路程有多长，客户端量不出来。

本章 Demo 因此在流式请求之外，还读取 vLLM 的请求级 timing metrics，并对请求前后的 `/metrics` 做差。请求级指标可以直接归属于这条请求；Prometheus 增量是直接观测到的服务端聚合变化，但只有在无其他流量时才近似对应当前请求。两种证据不能混为一谈。[vLLM Per-Request Metrics](https://docs.vllm.ai/en/latest/features/per_request_metrics/)
...
完整生命周期报告尚未生成。手头已有的是[第 1 章的 GX10 客户端流式报告](../ch01/evidence/gx10-streaming-client-report.json)，它在 GX10 的 NVIDIA GB10 上对 `Qwen/Qwen2.5-0.5B-Instruct` 发起一次真实流式请求，记录了 `prompt_tokens=30`、`output_tokens=256`、`stream_chunks=254`、首个内容 chunk 等待和 chunk 间隔。

这是一份真实数据，但它属于四级证据表中的第一级，只能回答客户端问题：
