...
第 1 章的 GX10 客户端流式报告提供了一组可以落到这条机制上的实际计数：`prompt_tokens=30`、`output_tokens=256`、`stream_chunks=254`。前两个来自服务端 usage，第三个是客户端计数。在标准逐 token 自回归路径下，可以这样解释：
...
本章先读取第 1 章真实 vLLM 报告中的 `prompt_tokens`、`output_tokens` 和 `stream_chunks`，把真实请求映射到 Prefill、Decode 和响应分片。报告路径是 `content/ch01/evidence/gx10-streaming-client-report.json`。这份报告只覆盖客户端观测层，因此它能约束计数关系，不能提供任何阶段耗时。
...
报告顶部会写出 `synthetic_mechanics`，也就是“合成机制报告”。这里的 synthetic 指张量形状和事件由公式构造，不是用模拟耗时替代真实 vLLM。真实请求计数来自第 1 章客户端流式报告；检查器只验证概念关系，不证明模型输出正确，也不提供性能 Benchmark。
...
打开第 1 章的 GX10 客户端流式报告，只读取这三个字段：`prompt_tokens=30`、`output_tokens=256`、`stream_chunks=254`。课堂任务不是评价速度，而是把它们放回本章的数据流。
...
3. 使用第 1 章 GX10 客户端流式报告，解释 30 个输入 tokens 和 256 个输出 tokens 如何映射到一次 Prefill 与后续生成。
