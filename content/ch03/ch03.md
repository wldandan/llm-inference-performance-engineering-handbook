# 第 3 章 Transformer 生成机制：模型怎样生成下一个 token
...
- 使用真实 vLLM 请求报告解释 Prefill、Decode、token 与 chunk 的关系，再用机制检查器核对张量形状、Sampling 候选和 KV Cache 增长事件。

第 2 章从请求视角定义了生命周期。本章暂时越过 Serving 组件边界，直接观察模型计算：token IDs 进入 Decoder-only Transformer 后，怎样一步步产生下一个 token？第 4 章再把这些计算放回 Gateway、Scheduler、Worker 与 Runtime 的职责关系中。

本章讲机制，不评价速度。TTFT、TPOT 和成功输出 token 速率怎样定义，放到第 5 章；算力、显存容量、显存带宽和 Kernel 为什么会限制这些计算，以及 Prefill/Decode 如何优化，都在后续 Part 和 Advanced 展开。
...
推理时会同时遇到三类数据。把它们分开，后面理解显存、并发和请求隔离会容易很多：

| 数据 | 生命周期 | 典型内容 | 是否随请求变化 |
|---|---|---|---|
| 模型参数 | 模型加载后长期存在 | Embedding、Attention、MLP、LM Head 权重 | 通常不随请求变化 |
| 临时激活 | 一次 forward 内产生和消费 | hidden states、Q/K/V、Attention 输出、Logits | 随 batch、序列和执行步骤变化 |
| 请求状态 | 从 Prefill 延续到 Decode | KV Cache、采样状态、已生成 token | 每条请求独立增长并在终态交接或回收 |

模型参数回答“模型学到了什么变换”，临时激活回答“这一轮正在算什么”，KV Cache 回答“这条请求已经处理过哪些历史位置”。KV Cache 由模型计算产生，却属于在线请求状态，不应和模型权重混成一类。

![从文本到下一个 token](figures/fig03-01_token_generation_pipeline.svg)

图3-1：从文本到下一个 token。

## 3.1 Tokenizer、Chat Template 与 Embedding
...
## 3.2 Decoder-only Transformer Block
...
![Decoder-only Transformer Block](figures/fig03-02_decoder_block.svg)
...
![Causal Self-Attention](figures/fig03-03_causal_self_attention.svg)

图3-3：Causal Self-Attention。

## 3.4 Multi-Head Attention 与 GQA
...
num_kv_heads        = 2
queries_per_kv_head = 14 / 2 = 7
```

因此一共有 2 个 KV Heads，每个 KV Head 被 7 个 Query Heads 共享。这里的 `7` 是每个 KV Head 对应的 Query Head 数，不是 KV 组数。若 batch 为 B、序列长度为 S、head dimension 为 D，逻辑形状是：
...
GQA 不代表只有两个 Attention Heads。Query 仍有 14 个 Head，只是需要保存和读取的 K/V Head 更少。它如何影响 KV Cache 容量和 Decode 带宽，会在后续 KV Cache 与 Decode 章节展开；这里先把结构关系讲清楚。

![MHA 与 GQA](figures/fig03-04_gqa_heads.svg)

图3-4：MHA 与 GQA。

## 3.5 Prefill：一次处理整段 Prompt
...
![Prefill](figures/fig03-05_prefill_execution.svg)

图3-5：Prefill。

## 3.6 Sampling：从 Logits 选择 token
...
温度较低时，较大的 Logit 会获得更集中的概率；温度较高时，概率分布更平。部分推理 API 把 Temperature 等于零约定为 Greedy Decoding；本章教学 Demo 为避免把“约定”混入 Softmax 数学，只接受大于零的 Temperature。需要 Greedy 时，直接选择最大 Logit，而不是执行除零。
...
![Sampling](figures/fig03-06_sampling_pipeline.svg)

图3-6：Sampling。

## 3.7 Decode：一次消费一个新 token
...
第 2 章的 GX10 客户端流式报告提供了一组可以落到这条机制上的实际计数：`prompt_tokens=30`、`output_tokens=256`、`stream_chunks=254`。前两个来自服务端 usage，第三个是客户端计数。在标准逐 token 自回归路径下，可以这样解释：

```text
30 个 Prompt tokens
  -> 1 次 Prefill
  -> 采样得到第 1 个输出 token
  -> 继续生成其余 255 个输出 tokens
  -> 客户端收到 254 个非空内容 chunks
```

这不是说服务端一定执行了 255 次彼此独立的 GPU 提交。Serving 引擎会组织 batch 和 iteration，某些生成技术也可能让一次模型执行接受多个 token。本章能确定的是模型输出 token 的逻辑依赖；引擎怎样调度这些计算属于第 4 章和后续优化章节。`254` 个 chunks 也再次说明，网络分片数不能替代模型 token 数。

![Decode 与 KV Cache](figures/fig03-07_decode_kv_loop.svg)

图3-7：Decode 与 KV Cache。

## 3.8 Demo：用真实请求约束机制检查

本章先读取第 2 章真实 vLLM 报告中的 `prompt_tokens`、`output_tokens` 和 `stream_chunks`，把真实请求映射到 Prefill、Decode 和响应分片。报告路径是 `content/ch02/evidence/gx10-streaming-client-report.json`。这份报告只覆盖客户端观测层，因此它能约束计数关系，不能提供任何阶段耗时。

`code/ch03/mechanics.py` 是辅助机制检查器，只依赖 Python 标准库。它不会加载模型，也不生成真机性能数据；它负责把配置关系转换成可核对的张量形状、采样候选和 KV Cache 长度变化。
...
报告顶部会写出 `synthetic_mechanics`，也就是“合成机制报告”。这里的 synthetic 指张量形状和事件由公式构造，不是用模拟耗时替代真实 vLLM。真实请求计数来自第 2 章客户端流式报告；检查器只验证概念关系，不证明模型输出正确，也不提供性能 Benchmark。

![机制检查器输出形状、采样与执行关系](figures/fig03-08_demo_mechanics_report.svg)

图3-8：机制检查器输出形状、采样与执行关系。

需要观察真实模型时，可以继续运行 `content/workshops/00-model-internals`。该 Workshop 会读取真实模型 config、核对运行时张量形状，并观察 past_key_values 随 Decode 增长。它需要 transformers 与 torch，适合作为本章的扩展实验。

## 3.9 课堂案例：把真实请求还原成模型动作

打开第 2 章的 GX10 客户端流式报告，只读取这三个字段：`prompt_tokens=30`、`output_tokens=256`、`stream_chunks=254`。课堂任务不是评价速度，而是把它们放回本章的数据流。

先画出一次 Prefill：30 个输入 token 经过 Embedding 和全部 Transformer Blocks，KV Cache 长度先覆盖 30 个历史位置，最后一个 Prompt 位置的 Logits 经过 Sampling 产生第一个输出 token。随后画出自回归循环：新 token 成为下一步输入，历史 K/V 被读取，新 K/V 被追加，再产生下一组 Logits。

课堂讨论：

1. 为什么 256 个输出 tokens 不等于 256 个 Decode steps？
2. 为什么 254 个 chunks 不能用于推导模型执行次数？
3. 这份报告没有模型内部张量，哪些机制结论来自 Transformer 定义，而不是请求报告的直接观测？

### 补充案例 A：Sampling 为什么会让回答分叉

对同一 Prompt 使用非零 Temperature 时，Logits 相同也不保证每一步选择同一个 token。某一步选出不同 token 后，后续上下文会变化，整段答案会继续分叉。

讨论问题：如果改成 Greedy Decoding，同样输入是否一定得到相同输出？还需要固定哪些模型、运行时和请求条件？
...
本章完成的是“理解系统”中的模型机制。下一章把这些计算放回 LLM Serving，解释 Gateway、Scheduler、Worker、Runtime 和 GPU 如何把一条请求组织成多轮模型执行。
...
- [ ] 能用真实请求中的 token 计数解释 Prefill 和 Decode，再说明机制检查器只能验证哪些关系。
...
3. 使用第 2 章 GX10 客户端流式报告，解释 30 个输入 tokens 和 256 个输出 tokens 如何映射到一次 Prefill 与后续生成。
4. 使用本章 Demo 比较 Temperature 0.5 与 1.5 的候选概率，不把结果解释成性能差异。
5. 把 Top-k 从 2 改成 4，观察哪些 token 获得非零概率。
6. 运行 `content/workshops/00-model-internals`，核对真实模型的 Query 与 K/V Head 形状，并把结果与机制检查器分开记录。
