...
![Scalability 在系统中的位置](figures/fig22-01_scalability_.svg)

图22-1：Scalability 在系统中的位置。

## 22.1 Scalability 在系统中的位置

Scalability 在系统中的位置 是本章的核心内容之一。写作时先说明它在 Scalability 流程中的位置，再说明输入、输出、关键状态和可观测证据。这样后续才能把指标变化和具体机制连起来。 这一节负责建立边界，不急着下优化结论。

![Capacity Model](figures/fig22-02_capacity_model.svg)

图22-2：Capacity Model。
...
![Memory Pool](figures/fig22-03_memory_pool.svg)
...
![KV Cache 容量](figures/fig22-04_kv_cache_.svg)
...
![Multi-GPU](figures/fig22-05_multi-gpu.svg)
...
![Scale-up 与 Scale-out](figures/fig22-06_scale-up_and_scale-out.svg)

图22-6：Scale-up 与 Scale-out。

## 22.6 Scale-up 与 Scale-out
...
![成本与容量边界](figures/fig22-07_and.svg)
...
![Demo：估算一个服务容量](figures/fig22-08_demo.svg)
...
![课堂案例：显存还够但并发上不去](figures/fig22-09_figure.svg)

图22-9：课堂案例：显存还够但并发上不去。
...
![Scalability 工作机制 的章节边界](figures/fig22-10_figure.svg)

图22-10：Scalability 工作机制 的章节边界。
