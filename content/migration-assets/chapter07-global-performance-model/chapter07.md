...
```text
TTFT ≈ ingress + gateway + queue + preprocess
       + prefill + sampling + first-chunk-egress

E2E  ≈ TTFT + remaining-decode + remaining-stream/egress
```

这里的第一个输出 token 来自 Prefill 产生的最后位置 Logits，再经 Sampling 选出；它不是额外执行一次 Decode forward 的结果。第一个 token 成为下一次模型前向的输入后，才进入后续 Decode 循环。不同框架可能把采样时间记入 Prefill、模型执行或输出处理，但不能因此在计算路径里凭空增加一个 Decode step。
