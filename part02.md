...
| 章节 | 作用 |
|---|---|
| Chapter 5 Benchmark Design | 建立可信 Baseline，说明 Warmup、Repeat、Prompt / Output / Concurrency 如何影响结论。 |
| Chapter 6 Profiling Toolchain | 说明 Nsight Systems、Nsight Compute、PyTorch Profiler、nvidia-smi 和 vLLM Profiling 分别能回答什么问题。 |
| Chapter 7 Root Cause Analysis | 从 Timeline、GPU Utilization、Memory Bandwidth、CPU 和 Scheduler 现象追溯根因。 |
| Chapter 8 Performance Diagnosis | 把前面的方法整理成 TTFT、TPOT、TPS、GPU Util 和 Memory 的统一诊断 Checklist。 |

这四章不能写成工具清单。第 5 章先保证实验输入可靠，第 6 章讲数据从哪里来，第 7 章讲如何把数据连成因果链，第 8 章才把方法压缩成现场可用的诊断流程。
