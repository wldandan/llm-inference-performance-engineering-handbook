# 第 4 章 Storyboard：LLM Inference Architecture

## 索引

| 图号 | 标题 | 核心问题 | 状态 |
|---|---|---|---|
| 图4-1 | LLM 推理系统全局架构 | 请求与响应分别经过哪些逻辑组件？ | final v2 |
| 图4-2 | Client 与 Gateway 边界 | 业务调用和服务入口分别负责什么？ | final v2 |
| 图4-3 | 三层流量决策 | Router、Admission、Engine Scheduler 如何分工？ | final v2 |
| 图4-4 | Worker、Runtime 与 Accelerator | 执行计划如何变成硬件计算？ | final v2 |
| 图4-5 | 推理服务的三类状态 | 请求、执行和资源状态由谁拥有？ | final v2 |
| 图4-6 | LLM Serving 部署形态 | 逻辑角色如何映射到不同部署单元？ | final v2 |
| 图4-7 | 成熟系统的架构层级映射 | 平台、推理服务器和 LLM Engine 有何区别？ | final v2 |
| 图4-8 | 从现象到组件和观测点 | 架构图如何指导证据采集？ | final v2 |
| 图4-9 | 架构契约 Demo | JSON 如何变成可验证的路径与部署报告？ | final v2 |
| 图4-10 | Chapter 4 的章节边界 | 架构与生命周期、模型机制、硬件和指标如何分工？ | final v2 |

## 图4-1 LLM 推理系统全局架构

- 对应正文：核心问题与第 4.1 节。
- 核心问题：请求从 Client 到 Accelerator，再如何返回？
- 核心观点：单实例最小路径由 Client、Gateway、Engine Scheduler、Worker、Runtime 和 Accelerator 组成；Router 与全局 Admission 是规模化时按需增加的职责。返回路径不重复做路由和准入。
...
- 关键结论：Router 与 Admission 按需增加；单实例仍要有入口、调度和执行责任。

## 图4-2 Client 与 Gateway 边界
...
## 图4-4 Worker、Runtime 与 Accelerator
...
## 图4-6 LLM Serving 部署形态

- 对应正文：第 4.6.2 节（Advanced 选读）。
- 核心问题：同一组逻辑职责可以怎样部署？
...
## 图4-10 Chapter 4 的章节边界

- 对应正文：第 4.10 节与本章总结。
- 核心问题：本章和前后章节分别回答什么？
- 核心观点：Chapter 2 给阶段，Chapter 3 给模型机制，Chapter 4 给组件，Chapter 5 给指标；硬件细节与规模化 Serving 留给 Advanced 内容。
- 不应出现：具体优化技术清单。
- 阅读路径：以 Chapter 4 为中心向前后展开。

```text
[Ch2 Lifecycle] -> [Ch3 Model Mechanics] -> [Ch4 Serving Architecture] -> [Ch5 Metrics]
                                                    |
                                                    v
                                          [Advanced Infrastructure]
```

- 图中文字：Lifecycle、Model Mechanics、Component Ownership、Metrics、Advanced Infrastructure。
- Key Takeaway：先回答“谁负责”，再研究“为什么慢”和“怎么优化”。

## 全局视觉规范

- 画布：16:9 SVG，`1280x720`。
- 风格：浅灰画布、白色内容卡、深色标题带与明确箭头，不使用装饰性图标。
- 颜色：只使用 Slate 黑、深灰、浅灰与白色；请求与响应用实线/虚线区分。
- 字体：系统无衬线字体，中文优先；标题 32px，节点 18–22px，说明不小于 14px。
- 每张图只保留一个主阅读方向，底部统一放“关键结论”深色条。
- 每张 SVG 都包含可访问标题、说明和 `chapter04-v2` 视觉契约标识，并配套 `figure-note.md`。
- 不把开源项目官方架构图复制到本书，只画本章自己的抽象映射。

## 总体验收清单

- [x] 图 4-1 到图 4-10 与正文引用一一对应。
...
- [x] 所有图号、正文节号和 Demo 路径均对应 Chapter 4。
