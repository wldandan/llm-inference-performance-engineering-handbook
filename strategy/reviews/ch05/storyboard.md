# 第 5 章 Storyboard：LLM 性能指标

本章插图围绕指标口径、测量边界和分母展开，不提前讲 Benchmark Design 或性能优化。

| 图号 | 标题 | 核心问题 | 状态 |
|---|---|---|---|
| 图5-1 | 指标先回答问题 | 不同角色的问题该看哪类指标？ | final v2 |
| 图5-2 | 客户端与服务端测量边界 | 同一个请求有哪些不等价时钟？ | final v2 |
| 图5-3 | TTFT、TPOT 与 ITL | 四种延迟覆盖哪段时间？ | final v2 |
| 图5-4 | 吞吐率与 Goodput | 产能与达标产能有何不同？ | final v2 |
| 图5-5 | P50、P95 与 P99 | 分位数怎样描述尾部？ | final v2 |
| 图5-6 | 资源指标与结果指标 | GPU 很忙是否等于用户体验好？ | final v2 |
| 图5-7 | 成本指标的分母 | 同一笔成本除以什么才有意义？ | final v2 |
| 图5-8 | 可比较的指标合同 | 两个数字可比较需要哪些共同条件？ | final v2 |
| 图5-9 | 指标合同检查器输出 | 公式样例怎样生成可审计报告？ | final v2 |
...
- 每张图包含 title、desc、`chapter05-v2` 和“关键结论”。

## 图5-1 指标先回答问题

- 来源：5.0。
- Wireframe：`[业务问题] -> [体验 | 产能 | 可靠性 | 资源 | 成本] -> [对应指标]`。
- 重点：TTFT/ITL/E2E、成功 output tokens/s 与 requests/s、Goodput、GPU/Queue、Cost Denominator 五组。
- 不出现：优化手段或指标阈值。
- 关键结论：先写问题，再选择指标与护栏。
...
- 来源：5.4。
- Wireframe：共享 Window 中若干请求和 tokens，分别进入三个分子；Goodput 还通过 Success + Quality + SLO Filter。
- 重点：三者共用墙钟分母但分子不同。
- 关键结论：Goodput 同时检查成功、结果质量与延迟 SLO。
...
- 来源：5.6。
- Wireframe：左侧 GPU/Memory/Bandwidth/Queue 经虚线连接右侧 TTFT/成功输出与请求速率/Goodput/Cost，中间标“需要因果证据”。
- 重点：资源指标是解释线索，不是结果代理。
...
- 重点：synthetic_client_metrics、nearest-rank、shared wall clock、成功输出与失败请求部分输出分开、NOT A BENCHMARK。
